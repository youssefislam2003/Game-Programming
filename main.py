...
pipeIMG = loadIMG('pipe.png', (80,500)) or createPlaceholder((80, 500), COLOR_PIPE)
...
def _draw_all():
     ...
     for pipe in pipes:
          scaled_pipe_top = pg.transform.scale(pipeImgFlipped, (80, pipe['h']))
          win.blit(scaled_pipe_top, (pipe['x'], 0))

          bottom_height = WIN_H - pipe['h'] - pipe['gap']
          scaled_pipe_bottom = pg.transform.scale(pipeIMG, (80, bottom_height))
           ...
           # Update pipes with new gap size
               ...
               for p in pipes:
                    ...
                    not (p['h'] < bird_y < p['h'] + p['gap'])) or bird_y + bird_h > WIN_H or bird_y < 0:
                         ...
                         lg.error("Collision detected, Game Over!")
